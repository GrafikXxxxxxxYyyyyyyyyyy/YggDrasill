...
def test_sdxl_refiner_objective_uses_aesthetic_time_ids(tmp_path) -> None:
    import torch
    ...
    objective = SDXLRefinerLoRAObjective(
    ...
    objective.compute_loss(
        {
            ...
            "aesthetic_score": torch.tensor([7.5]),
        }
    )

    assert components.unet.last_time_ids is not None
    assert tuple(components.unet.last_time_ids.shape) == (1, 5)
    assert float(components.unet.last_time_ids[0, -1]) == pytest.approx(7.5)
